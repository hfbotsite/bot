...
class Connector():
	...
	async def set_margin_mode(self, symbol, margin_mode, leverage):
		if isinstance(self.exchange, ccxt.binance):
			market = symbol
		else:
			market = symbol + ':' + symbol.split('/')[1]
	...
	# Post a market order
	async def post_market_order(self, symbol, side, amount, leverage, reason):
	...
	# Get all orders
	async def get_all_orders(self, symbol, type, id):
		#p = []
		#print(int(time.time()))
		q = None
		...
		if isinstance(self.exchange, ccxt.kucoinfutures) or isinstance(self.exchange, ccxt.okx) or isinstance(self.exchange, ccxt.bybit):
			market = symbol + ':' + symbol.split('/')[1]
			if isinstance(self.exchange, ccxt.okx) and type == 'stop_loss':
				...
				q_canceled = await self.exchange.fetch_canceled_orders(symbol=market, limit=1, params=canceled_par)
				await asyncio.sleep(0.1)

				q = q_open + q_closed + q_canceled
	...
	# Get an order
	async def get_order(self, order_id, symbol, type_ord):
		qq = None
		if isinstance(self.exchange, ccxt.binance):
			market = symbol
		else:
		...
		try:
			if isinstance(self.exchange, ccxt.binance) or isinstance(self.exchange, ccxt.bingx) or (isinstance(self.exchange, ccxt.okx) and type=='market'):
				...
				q = await self.get_all_orders(symbol, type_ord, order_id)
				#print(q)
				for item in q:
					if item['id'] == order_id:
						qq = item
	...
	# Return available balance of an asset
	async def get_balance(self, b_pair, q_pair, config):
		MARKETS = []
		...
		if isinstance(self.exchange, ccxt.bybit):
			# ss = b_pair + '/' + q_pair + ':' + q_pair
			# qqqq = await self.exchange.fetch_trades(symbol=ss, limit=2, params = {})
			# print(qqqq)
			...
			if qq[0]['entryPrice'] != None:
				collateral = qq[0]['initialMargin']
				pnl = float(qq[0]['info']['unrealisedPnl'])
				pos_average = float(qq[0]['entryPrice'])
				leverage = float(qq[0]['leverage'])
				side = qq[0]['side']
				pos_amount = float(qq[0]['contracts'])
				pos_contract_size = float(qq[0]['contractSize'])
